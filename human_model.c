...
initialize() {
    ...
    defineConstant("Q", 100);

    defineConstant("burn_in", 1.0);
    defineConstant("generation_time", 2500);
    //defineConstant("trees_file", "/tmp/tmp9zw1ieca.ts");
...
1 early() {
	...
	community.registerLateEvent(NULL,
		...
		"c(ind0,ind1).outputVCF(filePath=\""+output+".vcf\""+", simplifyNucleotides=T);}",
		g, g);					
